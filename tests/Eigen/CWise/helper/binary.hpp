#ifndef TESTS_MODULES_EIGEN_CWISE_HELPER_BINARY_HPP
#define TESTS_MODULES_EIGEN_CWISE_HELPER_BINARY_HPP
...
/**
 * @brief pointX = MatrixBase, pointY = MatrixBase
 */
    ...
    Eigen::MatrixBase<DY> const& targetDerivY, double prec)
{
    ...
    auto const size = targetValue.size();
    WHEN("pushing forward tangent from x")
    {
        operandX.derivative() = Eigen::MatrixXd::Identity(size, size);
        operandY.derivative() = Eigen::MatrixXd::Zero(size, size);
    ...
    {
        operandX.derivative() = Eigen::MatrixXd::Zero(size, size);
        operandY.derivative() = Eigen::MatrixXd::Identity(size, size);
    ...
    {
        expression._pullBack(Eigen::MatrixXd::Identity(size, size));
...
/**
 * @brief pointX = MatrixBase, pointY = double
 */
template <typename ValueX, typename ValueY, typename Derivative,
    typename Derived, typename T, typename DX, typename DY>
void checkBinaryOp(test::MockOperation<ValueX, Derivative>& operandX,
    test::MockOperation<ValueY, Derivative>& operandY,
    AutoDiff::Expression<Derived>& expression,
    Eigen::MatrixBase<T> const& pointX, double pointY,
    Eigen::MatrixBase<T> const& targetValue,
    Eigen::MatrixBase<DX> const& targetDerivX,
    Eigen::MatrixBase<DY> const& targetDerivY, double prec)
{
    operandX.value() = pointX;
    ...
    {
        operandX.derivative() = Eigen::MatrixXd::Identity(size, size);
        operandY.derivative() = Eigen::RowVectorXd::Zero(size);
        auto const derivativeX{expression._pushForward()};
        THEN("operation yields correct derivative")
        {
            CAPTURE(derivativeX, targetDerivX);
    ...
    WHEN("pushing forward tangent from y")
    {
        operandX.derivative() = Eigen::VectorXd::Zero(size);
        operandY.derivative() = Eigen::MatrixXd::Identity(1, 1);
        auto const derivativeY{expression._pushForward()};
        THEN("operation yields correct derivative")
        {
            CAPTURE(derivativeY, targetDerivY);
...
/**
 * @brief pointX = double, pointY = MatrixBase
 */
template <typename ValueX, typename ValueY, typename Derivative,
    typename Derived, typename T, typename DX, typename DY>
void checkBinaryOp(test::MockOperation<ValueX, Derivative>& operandX,
    test::MockOperation<ValueY, Derivative>& operandY,
    AutoDiff::Expression<Derived>& expression, double pointX,
    Eigen::MatrixBase<T> const& pointY, Eigen::MatrixBase<T> const& targetValue,
    Eigen::MatrixBase<DX> const& targetDerivX,
    Eigen::MatrixBase<DY> const& targetDerivY, double prec)
{
    operandX.value() = pointX;
    ...
    WHEN("pushing forward tangent from x")
    {
        operandX.derivative() = Eigen::MatrixXd::Identity(1, 1);
        operandY.derivative() = Eigen::VectorXd::Zero(size);
        auto const derivativeX{expression._pushForward()};
        THEN("operation yields correct derivative")
        {
            CAPTURE(derivativeX, targetDerivX);
    ...
    WHEN("pushing forward tangent from y")
    {
        operandX.derivative() = Eigen::RowVectorXd::Zero(size);
        operandY.derivative() = Eigen::MatrixXd::Identity(size, size);
        auto const derivativeY{expression._pushForward()};
        THEN("operation yields correct derivative")
        {
...
#endif // TESTS_MODULES_EIGEN_CWISE_HELPER_BINARY_HPP
