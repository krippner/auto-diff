#ifndef TESTS_MODULES_EIGEN_CWISE_HELPER_UNARY_HPP
#define TESTS_MODULES_EIGEN_CWISE_HELPER_UNARY_HPP
    ...
    Eigen::MatrixBase<D> const& targetDeriv, double prec)
{
    ...
    {
        operand.derivative() = Eigen::MatrixXd::Zero(size, size);
 ...
 * @brief point = double
 */
template <typename Value, typename Derivative, typename Derived, typename V,
    typename D>
void checkUnaryOp(test::MockOperation<Value, Derivative>& operand,
    AutoDiff::Expression<Derived>& expression, double point,
    ...
    Eigen::MatrixBase<D> const& targetDeriv, double prec)
{
    ...
    WHEN("pushing forward tangent")
    {
        operand.derivative() = Eigen::MatrixXd::Identity(1, 1);
        auto const opDerivative{expression._pushForward()};
        THEN("operation yields correct derivative")
        {
            CAPTURE(opDerivative, targetDeriv);
    ...
    WHEN("pulling back gradient")
    {
        operand.derivative() = Eigen::VectorXd::Zero(size);
        expression._pullBack(Eigen::MatrixXd::Identity(size, size));
        auto const opDerivative{operand.derivative()};
        THEN("operation yields correct derivative")
        {
...
#endif // TESTS_MODULES_EIGEN_CWISE_HELPER_UNARY_HPP
